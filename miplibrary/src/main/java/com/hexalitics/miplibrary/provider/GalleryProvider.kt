package com.hexalitics.miplibrary.provider
...
import com.hexalitics.miplibrary.ImagePickerActivity
import com.hexalitics.miplibrary.R
import com.hexalitics.miplibrary.util.IntentUtils
import com.hexalitics.miplibrary.util.PermissionUtil
...
class GalleryProvider(
    ...
    init {
        ...
        maxCount = bundle.getInt(ImagePicker.EXTRA_MAX_COUNT, 10)
    }
    ...
    private fun startGalleryIntent() {
        if (mMultipleAllowed) {
            TedImagePicker.with(this)
                ...
                .max(maxCount, "You can select up to $maxCount images")
    ...
    private fun handleResult(data: Intent?) {
        data?.clipData?.let { dataClipData ->
            if (dataClipData.itemCount > maxCount) {
                Toast.makeText(this, "You can select up to $maxCount images", Toast.LENGTH_SHORT).show()
                /*setError(R.string.error_failed_pick_gallery_image)
                return*/
            } else {
                fileList = ArrayList<Uri>()
